...
const char *IOT_TOPIC_PUBLISH_DEVICE = "$baidu/iot/shadow/ESP8266_V1d0/method/device/resp";
...
void parse_mqtt_callback(char *src_string)
{
	...
	if(!strcmp(docu["methodName"],"doFirmwareUpdate"))
	{
		const char* method = docu["methodName"]; 
		const char* requestId = docu["requestId"]; 
		JsonObject  payload = docu["payload"];
		const char* jobId = payload["jobId"]; 
		
		
		 String resp_update { "{\"methodName\":\"" + method +"\",\"requestId\":\"" +
					requestId + "\",\"status\":200}"};
		mqttclient.publish(IOT_TOPIC_PUBLISH_DEVICE, resp_update.c_str()); 
		
		 String req_update_start { "{\"requestId\":\"123456\"," +"{\"methodName\":\"reportFirmwareUpdateStart\"," + 
					"\"payload\":{\"jobId\":\"" + jobId + "\"}}"};
		mqttclient.publish(IOT_TOPIC_PUBLISH_REQ, req_update_start.c_str());
	}
...
void ticker_handler()
{
    if (!mqttclient.connected())
    {
        ...
        if (mqttclient.connect(IOT_CLIENT_ID, IOT_USERNAME, IOT_KEY))
        {
            ...
            const char *version = "{\"requestId\": \"12345\",\"methodName\": \"reportCurrentFirmwareInfo\", \"payload\": {\"firmwareVersion\": \"1.0.1\" }}";
